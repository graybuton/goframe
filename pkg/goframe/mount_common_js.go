...
func flushDirtyComponents() {
	...
	flushDirtyComponentsApplicationUpdate()
}

func renderDirtyComponents() float64 {
	started := performanceNow()
	mountedApp.rendering = true
...
func finishBrowserRender(kind string, started float64) {
	reportRender(kind, performanceNow()-started)
	flushPendingEffects()
